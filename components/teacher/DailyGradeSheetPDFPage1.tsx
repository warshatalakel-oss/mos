...
export default function DailyGradeSheetPDFPage1({ sheet, students, calculatedData, orientation, startingIndex }: PDFPageProps) {
    const { columnSettings, schoolName, teacherName, subjectName, gradeLevel, section, academicYear } = sheet;
    const s1_m1_cols = columnSettings.s1_m1;
    const s1_m2_cols = columnSettings.s1_m2;
    ...
    return (
        <div className={`${pageClasses} p-6 bg-white font-['Cairo'] flex flex-col`} dir="rtl">
            ...
            <h1 className="text-center text-lg font-bold">الدرجات اليومية للفصل الدراسي الاول - العام الدراسي {academicYear}</h1>
            <main className="flex-grow mt-2">
                <table className="w-full table-fixed border-collapse border-2 border-black text-xs">
                    <thead>
                        <tr className="bg-green-300 h-28">
                            ...
                            {/* Month 1 */}
                            {s1_m1_cols.map(c => <VerticalHeader key={c.id}>{c.name} ({c.maxGrade})</VerticalHeader>)}
                            ...
                            {/* Month 2 */}
                            {s1_m2_cols.map(c => <VerticalHeader key={c.id}>{c.name} ({c.maxGrade})</VerticalHeader>)}
                            ...
                            {/* Semester */}
                            <VerticalHeader className="bg-yellow-300">معدل الفصل الاول</VerticalHeader>
                            <VerticalHeader className="bg-yellow-300">نصف السنة</VerticalHeader>
                        </tr>
                        <tr className="bg-green-300">
                            <th className="border-2 border-black" colSpan={2}><LiftedCellContent>الشهر الأول</LiftedCellContent></th>
                            {s1_m1_cols.map(c => <PDFCell key={c.id}>{c.maxGrade}</PDFCell>)}
                            ...
                            {s1_m2_cols.map(c => <PDFCell key={c.id}>{c.maxGrade}</PDFCell>)}
                            <PDFCell className="bg-purple-100">100</PDFCell>
                            <PDFCell className="bg-purple-100">100</PDFCell>
                            <PDFCell className="bg-purple-100">-</PDFCell>
                            <PDFCell className="bg-yellow-100">-</PDFCell>
                            <PDFCell className="bg-yellow-100">-</PDFCell>
                        </tr>
                    </thead>
                    <tbody>
                        {students.map((student, index) => {
                            ...
                            return (
                                <tr key={student.id}>
                                    ...
                                    {/* Month 1 */}
                                    {s1_m1_cols.map(c => <PDFCell key={c.id}>{student.s1_m1_daily[c.id]}</PDFCell>)}
                                    <PDFCell className="bg-blue-100 font-bold">{calcs?.s1_m1_total_daily}</PDFCell>
                                    <PDFCell className="bg-blue-100">{student.s1_m1_written}</PDFCell>
                                    <PDFCell className="bg-blue-100 font-bold">{calcs?.s1_m1_avg}</PDFCell>
                                    {/* Month 2 */}
                                    {s1_m2_cols.map(c => <PDFCell key={c.id}>{student.s1_m2_daily[c.id]}</PDFCell>)}
                                    <PDFCell className="bg-purple-100 font-bold">{calcs?.s1_m2_total_daily}</PDFCell>
                                    <PDFCell className="bg-purple-100">{student.s1_m2_written}</PDFCell>
                                    <PDFCell className="bg-purple-100 font-bold">{calcs?.s1_m2_avg}</PDFCell>
                                    {/* Semester */}
                                    <PDFCell className="bg-yellow-100 font-bold">{calcs?.sem1_avg}</PDFCell>
                                    <PDFCell className="bg-yellow-100 font-bold">{student.midYear}</PDFCell>
                                </tr>
                            );
                        })}
                    </tbody>
